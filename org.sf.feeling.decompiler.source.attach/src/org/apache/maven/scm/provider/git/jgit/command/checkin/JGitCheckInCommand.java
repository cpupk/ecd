 ...
 * the default email domain to be used (will be used to create an email from the
 * username passed to maven):<br>
 * <code>git config --global maven-scm.maildomain mycomp.com</code> <br>
 * you can also enforce the usage of the username for the author and
 * committer:<br>
...
public class JGitCheckInCommand extends AbstractCheckInCommand implements GitCommand
{
	...
	private UserInfo getCommitter( ScmProviderRepository repo, Git git )
	{
		...
		{
			String defaultDomain = git.getRepository( ).getConfig( ).getString( GIT_MAVEN_SECTION,
					null,
					GIT_MAILDOMAIN );
