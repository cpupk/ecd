...
public class MethodUtils
{
	 ...
	 * classloader in a container, as the warning message will be emitted only
	 * once, not once per webapp. However making the warning appear once per
	 * webapp means having a map keyed by context classloader which introduces
	 * nasty memory-leak problems. As this warning is really optional we can
	 * ignore this problem; only one of the webapps will get the warning in its
	 * logs but that should be good enough.
	 */
	 ...
	 * container, this will affect all webapps. However making this configurable
	 * per webapp would mean having a map keyed by context classloader which may
	 ...
	 * methods of this class, and are never exposed to users of this class. This
	 * means that the WeakHashMap is used only as a mechanism for limiting the
	 * size of the cache, ie a way to tell the garbage collector that the
	 * contents of the cache can be completely garbage-collected whenever it
	 * needs the memory. Whether this is a good approach to this problem is
	 * doubtful; something like the commons-collections LRUMap may be more
	 * appropriate (though of course selecting an appropriate size is an issue).
	 * <p>
	 ...
	 * MethodDescriptor.equals method. So two components that load the same
	 ...
	 * 
	 * @param cacheMethods
	 *            <code>true</code> if methods should be cached for greater
	 ...
	 * that match and then executes the first it finds with compatable
	 * parameters.
	 * </p>
	 * 
	 * <p>
